...
import usersModelService from '../models/User.model.js';
...
const initializePassport = () =>{
    passport.use('register', new LocalStrategy({passReqToCallback:true,usernameField:'email',session:false}, async(req,email,password,done)=>{
        try{
            const {name} = req.body;
            const exists = await usersModelService.findOne({email});
            ...
            let result = await usersModelService.create(newUser);
    ...
    passport.use('login',new LocalStrategy({usernameField:"email",session:false},async(email,password,done)=>{
        try{
            const user = await usersModelService.findOne({email});
    ...
    passport.deserializeUser(async(id,done)=>{
        let result = await usersModelService.findOne({_id:id})
        return done(null,result);
    })
